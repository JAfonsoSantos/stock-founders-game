-- Allow game owners to insert demo users for their games
CREATE POLICY "Game owners can insert demo participants" 
ON public.users 
FOR INSERT 
WITH CHECK (
  EXISTS (
...
-- Update the add_demo_participant function to bypass RLS for demo user creation
DROP FUNCTION IF EXISTS public.add_demo_participant;
...
BEGIN
  ...
  -- Insert demo user (this will now work with the new policy)
