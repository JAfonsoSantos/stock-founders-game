...
-- Add circuit breaker fields to games table if not exists
ALTER TABLE games ADD COLUMN IF NOT EXISTS circuit_breaker_active BOOLEAN DEFAULT FALSE;
ALTER TABLE games ADD COLUMN IF NOT EXISTS circuit_breaker_until TIMESTAMP WITH TIME ZONE;

-- Function to check for circuit breaker conditions
...
BEGIN
    ...
    IF game_record.circuit_breaker_active AND 
       game_record.circuit_breaker_until > NOW() THEN
        RAISE EXCEPTION 'Trading is paused due to circuit breaker until %', 
            game_record.circuit_breaker_until;
    END IF;
    ...
    IF previous_price IS NOT NULL AND previous_price > 0 THEN
        ...
        IF price_change_pct > 200 THEN
            ...
            RAISE NOTICE 'Circuit breaker activated for game % due to %% price change', 
                NEW.game_id, price_change_pct;
        END IF;
    END IF;
...
-- Create trigger for circuit breaker (before trade is recorded)
DROP TRIGGER IF EXISTS circuit_breaker_check ON trades;
CREATE TRIGGER circuit_breaker_check
    BEFORE INSERT ON trades
    FOR EACH ROW
    EXECUTE FUNCTION check_circuit_breaker();

-- Function to reset circuit breaker when time expires
CREATE OR REPLACE FUNCTION reset_expired_circuit_breakers()
RETURNS void AS $$
BEGIN
    UPDATE games 
...
-- Create a notification type for circuit breaker
-- (This will be used by the frontend to show alerts)