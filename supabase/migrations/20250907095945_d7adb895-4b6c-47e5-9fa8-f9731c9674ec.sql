...
CREATE OR REPLACE FUNCTION public.create_primary_order(
  p_game_id UUID,
  p_startup_id UUID,
  p_qty INTEGER,
...
CREATE OR REPLACE FUNCTION public.decide_primary_order(
  p_order_id UUID,
  p_decision TEXT,
...
-- Update calculate_vwap3_for_startup function with proper search_path
CREATE OR REPLACE FUNCTION public.calculate_vwap3_for_startup(startup_uuid uuid)
RETURNS numeric
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
